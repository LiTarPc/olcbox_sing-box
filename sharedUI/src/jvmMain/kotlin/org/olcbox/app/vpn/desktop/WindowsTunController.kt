...
internal class WindowsTunController(
    ...
    suspend fun start(
        ...
        socksSettings: DesktopSocksProxySettings = DesktopSocksProxySettings()
    ): Process {
        ensureAdministratorOrRequestRestart()

        val bypassDomains = socksSettings.bypassExtraList
            .lines()
            .map { it.trim() }
            .filter { it.isNotBlank() && !it.startsWith("#") }
        val extraBypassDomains = bypassDomains.filter { !it.contains("/") }
        val extraBypassCidrs = bypassDomains.filter { it.contains("/") }

        val configJson = SingboxConfigGenerator.generate(
            socksPort = socksPort,
            bypassRuEnabled = socksSettings.bypassRuEnabled,
            extraBypassDomains = extraBypassDomains,
            extraBypassCidrs = extraBypassCidrs,
            logLevel = "warning",
            logPath = singboxBinary.parent.resolve("singbox.log").toString()
        )

        val configFile = singboxBinary.parent.resolve("singbox-config.json")
        Files.writeString(configFile, configJson)

        val command = listOf(
            singboxBinary.toString(),
            "run",
            "--config",
            configFile.toString()
        )

        addLog("Starting sing-box TUN mode with auto routing and DNS hijack")
        val process = ProcessBuilder(command)
            .directory(singboxBinary.parent.toFile())
            .redirectErrorStream(true)
            .start()

        try {
            waitForAdapter(process)
            addLog("Windows TUN connected via sing-box")
            return process
        } catch (e: Exception) {
            stopProcess(process)
            throw e
