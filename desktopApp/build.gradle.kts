...
val hasCCompiler = provider {
    val path = System.getenv("PATH") ?: ""
    val pathSeparator = if (currentBuildOs.isWindows) ";" else ":"
    val extensions = if (currentBuildOs.isWindows) listOf(".exe", ".cmd", ".bat") else listOf("")
    val compilers = listOf("gcc", "clang")
    
    path.split(pathSeparator).any { dir ->
        val dirFile = File(dir)
        dirFile.isDirectory && compilers.any { compiler ->
            extensions.any { ext ->
                File(dirFile, compiler + ext).exists()
            }
        }
    }
}.get()
...
if (hasCCompiler) {
    desktopNativeAssetTasks.add(buildOlcRtcLibDarwinArm64)
    desktopNativeAssetTasks.add(buildOlcRtcLibDarwinAmd64)
    desktopNativeAssetTasks.add(buildOlcRtcLibLinuxAmd64)
    desktopNativeAssetTasks.add(buildOlcRtcLibLinuxArm64)
    desktopNativeAssetTasks.add(buildOlcRtcLibWindowsAmd64)
}
...
if (currentBuildOs.isWindows) {
    ...
    val singboxWindowsOutput = generatedNativeResources.map {
        it.file("native/sing-box.exe")
    }

    val localWintun = project.file("src/main/resources/native/wintun.dll")
    val localSingbox = project.file("src/main/resources/native/sing-box.exe")
    val localTun2Socks = project.file("src/main/resources/native/tun2socks-windows-amd64.exe")
    ...
    val extractTun2SocksWindowsAmd64 = if (localTun2Socks.exists()) {
        tasks.register<Copy>("extractTun2SocksWindowsAmd64") {
            from(localTun2Socks)
            into(tun2SocksWindowsOutput.get().asFile.parentFile)
        }
    } else {
        tasks.register<ExtractZipEntryTask>("extractTun2SocksWindowsAmd64") {
    ...
    val extractWintunWindowsAmd64 = if (localWintun.exists()) {
        tasks.register<Copy>("extractWintunWindowsAmd64") {
            from(localWintun)
            into(wintunWindowsOutput.get().asFile.parentFile)
        }
    } else {
        tasks.register<ExtractZipEntryTask>("extractWintunWindowsAmd64") {
    ...
    val downloadSingboxWindowsAmd64 = tasks.register<DownloadFileTask>("downloadSingboxWindowsAmd64") {
        ...
        outputFile.set(layout.buildDirectory.file("tmp/singbox/sing-box-windows-amd64-$singboxVersion.zip"))
    }

    val extractSingboxWindowsAmd64 = if (localSingbox.exists()) {
        tasks.register<Copy>("extractSingboxWindowsAmd64") {
            from(localSingbox)
            into(singboxWindowsOutput.get().asFile.parentFile)
        }
    } else {
        tasks.register<ExtractZipEntryTask>("extractSingboxWindowsAmd64") {
